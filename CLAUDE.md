...
There are two Python components: `plugins/aimi-engineering/hooks/` (the hook dispatcher and guards) and the modules beside `aimi-cli.sh` — `scripts/roadmap.py` (the roadmap.json document logic behind the `roadmap-*` verbs), `scripts/story_merge.py` (everything `story-merge` does after its flags are parsed) and `scripts/sanitize.py` (the one prose sanitizer both of them import, so neither owns it). Everything else has no build step, no lint step, no package manager and stays plain Bash. Both pytest suites require Python 3.10+ and `pytest` installed via `pip install pytest`.

**`python3` is a runtime requirement, not only a test-time one.** Claude Code already invokes `hooks/*.py` directly on every session, and `aimi-cli.sh`'s roadmap verbs shell out to `roadmap.py` while `story-merge` shells out to `story_merge.py`. `check_python3()` in `aimi-cli.sh` fails with an install hint, and it is called by those verbs only — every other verb stays pure Bash + jq and works on a host without python3. On OpenCode this is new: `install.sh` copies `scripts/` wholesale (`cp -R "$src/."`), so the files travel with no installer change, but the host now needs python3 to run a roadmap verb or `story-merge`.

- Run `python3 -m pytest plugins/aimi-engineering/scripts/tests/ -q` after any change to `roadmap.py`, `story_merge.py` or `sanitize.py`. Its `tests/golden_from_jq.json` was captured from the jq implementations that used to live in `aimi-cli.sh`, **before they were deleted**, and is the evidence that the port changed nothing. Never regenerate it from the Python — that would turn it into a snapshot of whatever the code does today. When a rule genuinely changes, the golden file changes in the same commit, with the reason in the message.
...
- **Run `test-tasks-concurrency.sh` after any change to the locking or the ordering of reads and writes in `cascade-skip`, `gate-pass`, `gate-fail` or `reset-orphaned`** — the four tasks.json verbs that read a precondition outside the lock they then write under. It runs two verbs at once and asserts what the loser sees. **Serial-only and not parallel-safe**, for the same reason `test-worktree-manager.sh` is: every assertion depends on a timing window, and the widest of them exists only because `cascade-skip`'s unlocked transitive-closure jq is slow (~2.8s at 400 stories on a 16-core host). Running it beside anything else compresses that window unpredictably. It is deliberately **not** in `test-aimi-cli.sh`'s `PARTS` array and so contributes nothing to `EXPECTED_ASSERTIONS`; it prints its own passed/failed totals and exits non-zero on failure. Fixtures live under `mktemp -d` and it never touches the repo's own `.aimi/`.
  - Its cascade-skip assertions are **green against a known defect, on purpose**: a story that completes inside the unlocked window is overwritten with `status: "skipped"`. Read the comment above `test_cascade_skip_loses_a_concurrent_completion` before changing them — those assertions invert when the defect is fixed, and that inversion is meant to be the reviewable statement that behaviour changed. The `reset-orphaned` divergence in the same file is ranked separately and deliberately: its locked write re-selects `status == "in_progress"`, so only the printed report can be stale — cosmetic, not data loss.
