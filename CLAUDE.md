...
**What is still `jq` in `aimi-cli.sh`, and the principle that decides it, is written down once — in `plugins/aimi-engineering/CLAUDE.md` § What stays `jq` in aimi-cli.sh, and why.** It carries the organising sentence, the measured per-family census with its as-of marker, and the reason each surviving family stays. Read it there rather than re-scanning the script, and do not restate it here: this file and that one have drifted before by stating the same thing twice.
