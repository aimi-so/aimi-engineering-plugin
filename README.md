...
1. Creates or reuses the feature container. **The main working tree's checkout is left untouched for the entire run** — the one exception is `.gitignore`: a project's first container run appends a `.worktrees` entry automatically when the file is untracked or already has uncommitted changes of its own, but warns and leaves it alone instead of touching it when it's tracked and clean (add the entry yourself and commit it in that case).
...
5. On completion: stops the dev server, then removes the container while preserving the branch. Pushing `<branchName>` to `origin` first requires confirmation — an interactive session gets an AskUserQuestion prompt; an unattended (agent-mode) session pushes only when `--push` was passed, and otherwise skips it. Since nothing is left checked out locally, the report suggests `/aimi:open-pr --branch <branchName>` (which pushes on your behalf if needed) and `/aimi:review <branchName>` as next steps instead of the usual `gh pr create` / `/aimi:review`.

**Opt-in via `--container`/`--inline`:** `/aimi:plan` now writes `metadata.execution: "container"` into every freshly generated flat tasks.json, so a new plan runs in container mode by default. A tasks.json with `metadata.execution` absent — every file created before this feature shipped — still keeps running the inline flows described above exactly as before. Pass `--inline` on `/aimi:execute` (or `/aimi:next`) to opt a single invocation back into the inline flow, or `--container` to force container mode explicitly (mutually exclusive with each other); when the choice changes the mode, it's persisted back onto `metadata.execution` via `aimi-cli.sh set-execution-mode` so a later plain `/aimi:execute` continues in the same mode. On a phase-scoped tasks file (`metadata.phase` present), `--container`/`--inline` is ignored with a warning — a claimed phase always runs inside its own phase container regardless of the flag, and `metadata.execution` never applies there in the first place.
