...
**Opt-in via `--container`/`--inline`:** a tasks.json with no `metadata.execution` field — every file created before this feature, and any new one `/aimi:plan` generates — keeps running the inline flows described above exactly as before; `/aimi:plan` never writes `"container"` on its own. Pass `--container` (or `--inline`) on `/aimi:execute` to select the mode for a single invocation (mutually exclusive with each other); when the choice changes the mode, it's persisted back onto `metadata.execution` via `aimi-cli.sh set-execution-mode` so a later plain `/aimi:execute` continues in the same mode. On a phase-scoped tasks file (`metadata.phase` present), `--container`/`--inline` is ignored with a warning — a claimed phase always runs inside its own phase container regardless of the flag, and `metadata.execution` never applies there in the first place.
