...
- **A `creates`/`needs` `identity` is passed through VERBATIM** — never sanitized, never trimmed, never truncated. See `commands/references/sanitization.md` § *Contract Entries*: it is grepped literally by `verify-creates`, so any rule that rewrites it turns it into a name the phase will never deliver, and `roadmap-init` can no longer tell that it happened. An identity that will not survive the rules below must be RENAMED, not repaired. The `description` beside it is ordinary prose and takes the full sanitizer and the 500-char cap; an identity longer than 500 characters is refused rather than clipped, for the same reason.
...
- **Never author `;`, `|`, `&`, a backtick or a bare `$` into a `creates`/`needs` `identity`.** `roadmap-init` refuses the whole payload when one appears there, because `verify-creates` greps the identity as a literal string. Put that text in the `description` instead, where it is allowed: identity `cmd_clean` with description `does x; then y` is legal, identity `cmd_a;b` is not. Do **not** "fix" such an entry by deleting the character — that silently produces a different identity than the one the phase will actually deliver, which is the failure mode this whole contract exists to prevent. Rename the artifact, or move the text into the description.
