...
- **`creates` and `needs` entries are passed through VERBATIM** — never sanitized here. See `commands/references/sanitization.md` § *Contract Entries*: the identity half is grepped literally by `verify-creates`, so any rule that deletes content turns it into a name the phase will never deliver, and `roadmap-init` can no longer tell that it happened. Apply only the 500-char cap, and shorten the description rather than the identity if an entry exceeds it.
...
- **Never author `;`, `|`, `&` or a bare `$` into a `creates`/`needs` IDENTITY** — the part before the first `(`. `roadmap-init` refuses the whole payload when one appears there, because `verify-creates` greps the identity as a literal string. Put that text in the parenthesised description instead, where it is allowed: `cmd_clean (does x; then y)` is legal, `cmd_a;b (…)` is not. Do **not** "fix" such an entry by deleting the character — that silently produces a different identity than the one the phase will actually deliver, which is the failure mode this whole contract exists to prevent. Rename the artifact, or move the text.
