...
**`creates` and `needs` are not free-text fields.** Each entry is `{identity, description}`, and the two halves take opposite treatment — see `commands/references/sanitization.md` § *Contract Entries*. Pass each `identity` through **verbatim**: no sanitization, no trimming, no truncation. Let `roadmap-init` refuse a malformed one; the repair-and-retry rule further down this section exists for exactly that. Sanitizing an identity here does not duplicate the CLI's guard, it defeats it — the CLI can only refuse what it is given, so an identity you already damaged is one it accepts and nothing downstream can ever resolve. Stripping `<T>` from `parseList<T>` here yields `parseList`, which the CLI stores without complaint. The `description` beside it IS free text and takes every rule below, including the 500-char cap.
