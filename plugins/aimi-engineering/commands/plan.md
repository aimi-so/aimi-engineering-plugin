...
**`creates` and `needs` are not free-text fields.** They are carved out of every rule below — see `commands/references/sanitization.md` § *Contract Entries*. Pass each entry through **verbatim**, apply only the 500-char cap, and let `roadmap-init` refuse a malformed one; the repair-and-retry rule further down this section exists for exactly that. Sanitizing a contract entry here does not duplicate the CLI's guard, it defeats it: the guard compares what you submitted against its own normalized form, so an entry you already damaged makes both sides agree on the damage. Stripping `<T>` from `parseList<T>` here yields `parseList`, which the CLI stores without complaint and which nothing downstream can ever resolve.
