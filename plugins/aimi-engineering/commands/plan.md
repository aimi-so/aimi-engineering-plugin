...
**If `roadmap-init` exits non-zero, first check whether the failure is one this command authored and can repair.** A stderr beginning `Error: roadmap-init: malformed creates/needs identity` names, per offending entry, the phase, the list, the quoted entry and the specific reason — and every one of those entries came from `sanitizedPhases`, which this command composed and still holds in working memory. Repair them in `sanitizedPhases` exactly as each reason directs (move the offending text into the parenthesised description, drop a query string from an endpoint, rename an identity the sanitizer would rewrite), then **re-run the same call once**. If the retry also fails, fall through to the warn-and-continue path below; do not retry a second time.
