...
- Use the AskUserQuestion tool to clarify. Ask **2-3 questions per round**, then wait for answers.
...
Read `${CLAUDE_PLUGIN_ROOT}/skills/frontend-design/references/index.md` to resolve the register (brand vs product) and identify which reference files apply to this surface. Then read the appropriate register file (`brand.md` or `product.md`) and any additional references matched by the topic keywords.
...
Based on the brief, list which reference files under `${CLAUDE_PLUGIN_ROOT}/skills/frontend-design/references/` would be most valuable during implementation (e.g., `spatial-design.md` for complex layouts, `motion-design.md` for animated features, `interaction-design.md` for form-heavy features).
