...
## Phase Container Paths Per Project Group (Phase Mode)

When `PHASE_MODE=true`, each participating repository gets its own phase container instead of one shared across the whole phase: `PHASE_CONTAINER_PATHS[group_key] = project_roots[group_key] + "/.worktrees/" + PHASE_BRANCH` for every project group with at least one story scheduled this phase — keyed identically to `project_roots[group_key]` and to `CONTAINER_PATHS[group_key]` above, never a single global `PHASE_CONTAINER_PATH` scalar.

With exactly one group key — today's single-repo case — `PHASE_CONTAINER_PATHS[group_key]` has exactly one entry, at exactly today's `PHASE_CONTAINER_PATH` path, and every rule in this document reads exactly as it does today: this is the unchanged special case, and it is what keeps entries 05-10 of this feature from being a behavior change for existing single-repo users.

`EXEC_BRANCH[group_key]` is **not** a parallel per-group map: it stays the single `PHASE_BRANCH` scalar, reused by every group, because each repository gets its own independent git ref of that one name rather than a differently-named branch. `commands/execute.md:379`'s Step 0.9 Pass 1 collision check keys on `(repo toplevel, branch)` rather than the branch name alone — which is why the same `PHASE_BRANCH` name legitimately existing in several repositories at once is not a collision.

Bootstrap a Container Dev Server below needs no change for this: `dev-server.json` already keys `serve start`/`serve status`/`serve url` state by the container's absolute resolved path, not by branch name alone, and already anticipates two callers whose container happens to share a branch name — so N per-repository phase containers all sharing one `PHASE_BRANCH` name are already safe there.
...
Git branch names are repository-global. Two parallel `/aimi:execute` sessions each running their own phase container would otherwise mint identical unqualified story branches (e.g. both minting `US-003`) and collide. Story worktree/branch names in phase mode are therefore `<PLAN_DISC>-<PHASE_BRANCH>-<story.id>` instead of `<branchName>-<story.id>` — qualified by the full phase branch, not just the phase id, so they stay collision-free even against a sibling phase container running concurrently for the same feature, and qualified ahead of that by the plan discriminator (`commands/execute.md` § Plan Discriminator). The two qualifiers compose rather than compete, because they separate different things: phase-qualification separates two concurrent phases, and it cannot separate two PLANS whose story ids both restart at `US-001` over one reused branch name — the orthogonal axis the discriminator adds.

### CWD For Every Worktree Operation

Every `$WORKTREE_MGR create`/`merge-all`/`remove`/`list` call for a phase's stories runs with CWD set to that group's own container, `PHASE_CONTAINER_PATHS[group_key]` — never `AIMI_ROOT`, never a `project_root` from `commands/execute.md`'s Multi-Repo Handling grouping, and never another group's container. With exactly one group key — today's single-repo case — this is exactly today's single `PHASE_CONTAINER_PATH`, unchanged. Two reasons for the rule itself:

1. **`merge-all` checks out its target branch against whatever repo its CWD belongs to.** `worktree-manager.sh`'s `GIT_ROOT` is computed per-invocation from CWD (`git rev-parse --show-toplevel`), and `merge-all <story-worktree-names> --into <target-branch>` issues a bare `git checkout <target-branch>` against that root. Run from `AIMI_ROOT` (or from another group's `project_root`), that would check the phase branch out onto that repository's main working tree — forbidden by the Main Working Tree Untouched Invariant below. Run from `PHASE_CONTAINER_PATHS[group_key]`, `GIT_ROOT` resolves to that group's phase worktree root instead, and its story worktrees nest at `PHASE_CONTAINER_PATHS[group_key]/.worktrees/<story-worktree-name>` — a pattern `worktree-manager.sh` already supports unmodified, since a linked worktree's own `git rev-parse --show-toplevel` returns its own path, not the main repo's.
2. **Every Bash call is an isolated shell** (Step 0). `PHASE_CONTAINER_PATHS[group_key]` does not persist across calls on its own — each call that needs it either `cd`s to it explicitly at the top of the call, or passes it via `git -C`/`$WORKTREE_MGR` arguments, exactly like `$AIMI_CLI`/`$WORKTREE_MGR` themselves are re-resolved per call.
...
- **`EXEC_BRANCH[group_key]`** — the branch every worktree is created `--from`, every `merge-all --into` targets, and every cleanup scan pattern (`"[PLAN_DISC]-[EXEC_BRANCH[group_key]]-US-*"`) matches.
- **`EXEC_OWNS_ROOT`** — a single run-wide scalar (not a map — it does not vary by `group_key` within one run), true when this run's own worktree-manager invocations created `EXEC_ROOT` as a container it is responsible for (container and phase mode), false when `EXEC_ROOT` is a pre-existing project checkout this run never created (inline mode). It stays this same single scalar even once phase mode derives one `PHASE_CONTAINER_PATHS[group_key]` per repository: every group's container is equally created by this run, so there is nothing for a per-group value to distinguish. Consumed today only by the post-merge visual-verification origin rewrite (see Step 4).
...
| Phase (`PHASE_MODE=true`) | `PHASE_CONTAINER_PATHS[group_key]` | `PHASE_BRANCH` |
...
The split row is not a new derivation — it is exactly what `commands/execute.md`'s Phase-Mode Paired Split section (Derive and Validate Split Branch Names, Create Split Worktrees, and Spawn Split Sub-Orchestrators) already pre-assigns to `PHASE_CONTAINER_PATHS[group_key]` (scoped to that split member's own `group_key`) / `PHASE_BRANCH` before each split sub-orchestrator starts. There is one such pair per active split file — N of them, not two: each is derived from that file's own `metadata.splitGroup.project` (or, for a legacy frontend/backend pair, from its own basename slug) and qualified by the outer `PHASE_BRANCH`, landing at `$PHASE_CONTAINER_PATHS[group_key]/.worktrees/<that member's split branch>`. Each sub-orchestrator's own Step 4 then derives `EXEC_ROOT`/`EXEC_BRANCH` from the Phase row using its own pre-set values, with no split-specific code path.
...
For the entire span of a phase's execution — from the moment it is claimed (Step 1.7) through the last wave's merge — EVERY participating repository's own `git rev-parse HEAD` and `git status --porcelain` never change, checked at that repository's own toplevel: `project_roots[group_key]` for each group, or `AIMI_ROOT` itself only when `AIMI_ROOT` is the sole repository — not just `AIMI_ROOT`'s. Every commit for the phase lands only on `PHASE_BRANCH` inside that group's own `PHASE_CONTAINER_PATHS[group_key]`. This holds automatically once every rule above is followed: nothing in phase mode ever `cd`s to `project_roots[group_key]` (or `AIMI_ROOT`, or runs a mutating git command against either), and Step 2's Main Repo Branch Setup is skipped entirely in phase mode (see Step 2). In a multi-repo layout `AIMI_ROOT` is not a git repository at all, so the invariant as phrased purely in terms of `AIMI_ROOT` has no subject there — it is the per-repository restatement above, not `AIMI_ROOT` alone, that holds in every layout.
...
**On a zero exit, the caller reports the resolved base and its reason** — regardless of whether `create` above just cut `EXEC_BRANCH` fresh, attached it to an existing branch, or silently reused an already-present worktree at that path: e.g. `Container for [EXEC_BRANCH] ready — branched from [base] ([reason])`, rendering `reason` verbatim as one of `resolve-base-branch`'s own values (`explicit-base`, `target-exists`, `stacked-on-current`, `default-branch`, `detached-head`) — never paraphrased into prose. This report is the caller's own responsibility, not `worktree-manager.sh`'s: `create_worktree()` prints its own `From: <base>` line only on the branch that creates a worktree fresh, and returns early — printing nothing — on the branch that finds one already there (see the reuse check near the top of `create_worktree()` in `worktree-manager.sh`). A caller that only echoed the create call's own stdout would show the base once, on first creation, and never again on any later reuse; emitting the line here instead, from `CONTAINER_BASE`/its reason — both already in the caller's hands from its own `resolve-base-branch` call above — makes every call site's report identical across the fresh-create and reuse paths alike.
