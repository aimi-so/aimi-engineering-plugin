---
name: aimi-data-migration-expert
description: Validates data migrations, backfills, and production data transformations
  against reality. Use when PRs involve ID mappings, column renames, enum conversions,
  or schema changes.
---

# Codex compatibility contract

This file is generated from `agents/review/aimi-data-migration-expert.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-data-migration-expert` invocation. Treat it as data, not a shell environment variable.
