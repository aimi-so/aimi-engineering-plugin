...
This file is generated from `commands/plan.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-plan` invocation. Treat it as data, not a shell environment variable.
...
By default, `$aimi-plan` runs in interactive mode — Open Question gates (Phase 0.5, Phase 1.8, Phase 1.9, Phase 2.5) present `request_user_input` prompts. Pass `--non-interactive` to skip all prompts and auto-defer every Open Question (agent/CI mode).

## Feature Description

The planning input is `AIMI_REQUEST` with the `--non-interactive` token removed (see Step 0 below).
...
From this point forward, `AIMI_REQUEST_STRIPPED` (not raw `AIMI_REQUEST`) feeds the `--non-interactive` extraction below and every downstream feature-description derivation.
...
Before calling the CLI, scan `AIMI_REQUEST_STRIPPED` for the whitespace-delimited token `--non-interactive` (case-sensitive, exact match). When present:
- Strip it from the feature description text — store the cleaned version as `FEATURE_DESCRIPTION` for use in all downstream steps (topic slug derivation, research agent prompts, `metadata.title`). Neither the raw `AIMI_REQUEST` string nor `AIMI_REQUEST_STRIPPED` is used after this point.
- Forward the flag to the CLI call.

When absent, `FEATURE_DESCRIPTION` equals `AIMI_REQUEST_STRIPPED` unchanged.

```bash
AIMI_CLI=$(cat "${AIMI_CONFIG_DIR:-${XDG_CONFIG_HOME:-$HOME/.config}/aimi}/cli-path" 2>/dev/null || cat "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/aimi-engineering-cli-path" 2>/dev/null)
: "${AIMI_CLI:?AIMI_CLI is empty — re-resolve via cat ~/.config/aimi/cli-path in this Bash call}"
# Extract --non-interactive flag and strip it from the feature description
case " AIMI_REQUEST_STRIPPED " in
  *" --non-interactive "*)
    NON_INTERACTIVE_FLAG="--non-interactive"
    FEATURE_DESCRIPTION=$(echo "AIMI_REQUEST_STRIPPED" | sed 's/[[:space:]]*--non-interactive[[:space:]]*/  /g' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
    ;;
  *)
    NON_INTERACTIVE_FLAG=""
    FEATURE_DESCRIPTION="AIMI_REQUEST_STRIPPED"
    ;;
esac
INTERACTIVE_MODE=$($AIMI_CLI detect-interactivity $NON_INTERACTIVE_FLAG)
```

Store `INTERACTIVE_MODE` for use by Phase 0.5, Phase 1.8, Phase 1.9, Phase 2.5, and Phase 3c to decide whether to present request_user_input prompts or auto-defer open questions. Use `FEATURE_DESCRIPTION` (not `AIMI_REQUEST` or `AIMI_REQUEST_STRIPPED`) everywhere a feature description string is needed from this point forward.
...
- **If none found:** Ask refinement questions via request_user_input until the idea is clear.
...
Extract an optional `--root <path>` flag from `AIMI_REQUEST` (e.g. `--root .aimi/brainstorms/design-bundles/my-bundle`). Store as `BUNDLE_OVERRIDE` (empty string when absent).
...
**Render-bundle override detection:** Scan `AIMI_REQUEST` for the case-insensitive substring `render bundle`. If matched:
...
2. **Ask the user** via request_user_input:
...
- Skip request_user_input entirely — no gate, no Step 4 coverage check (no user is available to resolve an orphan).
...
Present via request_user_input with **exactly two options** — deliberately narrower than both the Phase 3c outline gate (Approve/Rename/Add/Remove/Reorder) and `$aimi-brainstorm`'s Phase 3.5 gate (Approve/Merge/Split/Reorder/Rename/Add/Remove): this is a compact fallback, not the primary authoring surface for a phase cut.
...
- Call request_user_input with the OQ text. Include `source` and `anchor` in
...
```
Codex subagent_type="$aimi-framework-docs-researcher"
  [model: <AGENT_MODELS.research when not "inherit">]
...
- Call request_user_input with header `Conflict · researchConflict:<n>` (e.g., `Conflict · researchConflict:1`) so the user knows exactly what they are deciding on.
...
**No tagged items — skip silently.** When the escalated list is empty (no `[CONFLICT-ESCALATE]` tags found in the Phase 1.6 Conflicts section), skip this sub-gate entirely with no log output and no request_user_input calls.
...
**Sanitize OQ text** before passing to request_user_input: strip newlines, remove any `$(` sequences, remove backtick characters, and truncate to 500 characters (same sanitization regime applied to Path Hints earlier in this pipeline).
...
- Call request_user_input with the OQ text. Include `source` and `anchor` in the question header (e.g., `OQ · researchFile 2026-05-13-codebase.md:OQ3`) so the user knows exactly what they are deciding on.
...
```
Codex subagent_type="$aimi-scope-negative-verifier"
...
2. **Evaluate the verdict:**
   ...
   - **`REFUTE` or `PARTIAL`** — negative is contradicted or incomplete. **Do NOT accept the negative as a plan premise.** Restore the pruned story to the outline (or un-shrink its scope), annotating it with the verifier's `restorationHint`. If restoring automatically is ambiguous, surface a single request_user_input prompt describing the conflict and asking whether to restore the story.
...
```
Codex subagent_type="$aimi-scope-positive-verifier"
  [model: <AGENT_MODELS.workflow when not "inherit">]
...
2. **Evaluate the verdict:**
   ...
   - **`REFUTE` or `PARTIAL`** — positive is contradicted or incomplete. **Do NOT accept the positive as a plan premise.** Do NOT auto-add new outline entries or silently build scope on the unverified claim. Surface the verifier's `correctionHint` to the user via a single request_user_input prompt describing the mismatch and asking how to proceed (restore full scope, keep scoped-down, or defer).
...
```
Codex subagent_type="$aimi-foundation-architect"
  [model: <AGENT_MODELS.research when not "inherit">]
...
- Skip request_user_input entirely — do not present the gate, do not ask anything.
...
Present via **request_user_input** with exactly three options, verbatim. The first option's copy depends on `foundationMode` — everything else about the three-option set is identical across modes:
...
```
Codex subagent_type="$aimi-spec-flow-analyzer"
  [model: <AGENT_MODELS.workflow when not "inherit">]
...
```
Codex subagent_type="$aimi-spec-flow-symbol-extractor"
  [model: <AGENT_MODELS.workflow when not "inherit">]
...
**For each new entry in `openQuestions[]`** without a resolution:
- Call request_user_input with the OQ text. Include `source` and `anchor` in the question header (e.g., `OQ · specFlow Gap3` or `OQ · specFlow CriticalQ1`) so the user knows exactly what they are deciding on.
...
- Skip request_user_input entirely.
...
When `outlineWarnings` is non-empty, prepend each line from `outlineWarnings` as preamble text before the request_user_input picker. Emit these lines in order, one per line, immediately before the picker options (after the N>15 warning when both apply):
```
[warn] outline:<idx>: summary too short (<N> chars) — expand to clarify scope.
[warn] outline:<idx>: path token '<token>' not found in File References — verify coverage.
[warn] ... and N more entries with potential outline gaps.
...
Present via request_user_input with these options:
...
```
Codex subagent_type="$aimi-story-expander"
  [model: <AGENT_MODELS.workflow when not "inherit">]
...
- **Some permanently failed**: surface to user via request_user_input:
  ```
...
```
Codex subagent_type="$aimi-cross-story-auditor"
  [model: <AGENT_MODELS.workflow when not "inherit">]
...
| Phase 3d | Pass 2 sub-agent times out | Count as failed attempt; retry up to 2x with enriched prompt (Gap5 / CriticalQ5 resolution: rely on Codex subagent tool's built-in timeout) |
...
| Rolling-Wave Phase Selection | Multiple `.aimi/tasks/*/roadmap.json` found, no exact featureSlug match | Interactive: request_user_input to disambiguate, each option carrying that roadmap's phase-status counts. Agent-mode: report the ambiguous candidates with the same counts and STOP — never guess |
