...
description: Expands a single outline entry into a full user-story JSON object (schema
  v3.3) for the /aimi:plan two-pass pipeline. Receives one outline entry plus full
  outline context, research, resolved decisions, and optional specs; writes one staging
  JSON file. Story IDs are NEVER assigned here — story-merge handles US-NNN assignment
  and outline:NN remap.
---

# Codex compatibility contract

This file is generated from `agents/workflow/aimi-story-expander.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-story-expander` invocation. Treat it as data, not a shell environment variable.
