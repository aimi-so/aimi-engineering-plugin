...
description: Proposes a stack-adaptive, Clean-Architecture/DDD-informed layering,
  folder layout, and set of conventions as reviewable defaults for a repository. Use
  when the /aimi:plan or /aimi:brainstorm greenfield foundation gate detects a greenfield
  or near-empty repository and needs a prescriptive architecture proposal before any
  story is written.
---

# Codex compatibility contract

This file is generated from `agents/research/aimi-foundation-architect.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-foundation-architect` invocation. Treat it as data, not a shell environment variable.
