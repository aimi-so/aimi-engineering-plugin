...
> **Backward compatibility:** v2.2 is backward compatible with v2.1. Existing v2.1 files work without modification.
...
|-------|------|----------|-------------|
| `schemaVersion` | string | Yes | Schema version, currently "2.2" |
...
| `planPath` | string \| null | No | Path to source plan markdown file. `null` when generated by task-planner (no intermediate plan). |
...
1. `schemaVersion` must be "2.2" (or "2.1" for backward compatibility)
