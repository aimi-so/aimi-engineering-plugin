...
description: Reads DesignSpec.md, BusinessSpec.md, and chat files from a Claude Design
  handoff bundle and authors a single self-contained HTML prototype with a per-view
  tab switcher. Use when brainstorm or plan needs to generate a faithful prototype
  from an existing bundle without a prior prototype.
---

# Codex compatibility contract

This file is generated from `agents/research/aimi-bundle-prototype-author.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-bundle-prototype-author` invocation. Treat it as data, not a shell environment variable.
