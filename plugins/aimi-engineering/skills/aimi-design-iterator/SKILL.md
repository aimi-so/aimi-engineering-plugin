---
name: aimi-design-iterator
description: Iteratively refines UI design through N screenshot-analyze-improve cycles.
  Use PROACTIVELY when design changes aren't coming together after 1-2 attempts, or
  when user requests iterative refinement.
---

# Codex compatibility contract

This file is generated from `agents/design/aimi-design-iterator.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-design-iterator` invocation. Treat it as data, not a shell environment variable.
