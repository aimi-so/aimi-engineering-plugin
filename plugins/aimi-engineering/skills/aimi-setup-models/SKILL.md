...
description: Configure per-category model assignments interactively for Claude Code,
  OpenCode, or Codex while preserving every other host's settings.
---

# Codex compatibility contract

This file is generated from `commands/setup-models.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-setup-models` invocation. Treat it as data, not a shell environment variable.
...
Use **request_user_input** with **five questions in one call** — one per category. Each question's options are sourced from `AVAILABLE_MODELS_JSON`. The label on each option is the bare model id; the picker automatically appends an "Other" option for free-form input.
