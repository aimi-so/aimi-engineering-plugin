...
description: Final-pass polish for frontend interfaces — catches the small details
  that separate good work from great work. Use after a feature is functionally complete
  and you want to align it to the design system, fix visual inconsistencies, and bring
  everything to a consistent quality level.
---

# Codex compatibility contract

This file is generated from `commands/design/polish.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-design-polish` invocation. Treat it as data, not a shell environment variable.
...
Before using `AIMI_REQUEST` as a polish target, check whether the user passed `--non-interactive`:
...
Strip the token from `AIMI_REQUEST` so it is not treated as a target path:

```bash
POLISH_TARGET=$(echo "AIMI_REQUEST" | sed 's/--non-interactive//g' | sed 's/^[[:space:]]*//' | sed 's/[[:space:]]*$//')
```

Use `$POLISH_TARGET` wherever `AIMI_REQUEST` would have been used as the polish target for the rest of this command.
...
- `picker` — interactive user. Present questions using **request_user_input** (one per tool call).
...
**When all three conditions are met**, spawn the design iterator as a Codex subagent:

```
Codex subagent_type="$aimi-design-iterator"
