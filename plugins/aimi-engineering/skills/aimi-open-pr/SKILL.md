---
name: aimi-open-pr
description: Open a pull request with title and description derived from git commits
  and diff
---

# Codex compatibility contract

This file is generated from `commands/open-pr.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-open-pr` invocation. Treat it as data, not a shell environment variable.
...
case " AIMI_REQUEST " in
  *" --branch="*)
    ...
    ;;
  *" --branch "*)
    CURRENT_BRANCH=$(echo "AIMI_REQUEST" | sed -n 's/.*--branch[[:space:]]\+\([^ ]*\).*/\1/p')
...
**When `INTERACTIVE_MODE=picker` and `.decision == "ask"`:** Use **request_user_input**, building the options from `ACCOUNT_CHECK_JSON`:
