---
name: aimi-schema-drift-detector
description: Detects unrelated schema.rb changes in PRs by cross-referencing against
  included migrations. Use when reviewing PRs with database schema changes.
---

# Codex compatibility contract

This file is generated from `agents/review/aimi-schema-drift-detector.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-schema-drift-detector` invocation. Treat it as data, not a shell environment variable.
