...
This file is generated from `commands/deepen.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-deepen` invocation. Treat it as data, not a shell environment variable.
...
If `AIMI_REQUEST` contains a path, use it. Otherwise, auto-discover:
