...
Runs regardless of whether a brainstorm loaded this session, so a later `/aimi:plan` (or `/aimi:plan --phase <N>`) re-invocation can continue an already-materialized roadmap. When `.aimi/tasks/<feature-slug>/roadmap.json` exists, auto-selects the next pending phase whose `dependsOn` are all `completed` (numeric id order — `2` before `2.1` before `3`), or honors an explicit `--phase <N>` override, refusing before any Task spawns when ineligible. Exactly one phase is expanded per invocation; gates on `validate-contracts` (unmet needs always block; duplicate creates block-or-warn per `--agent-mode`); ingests completed phases' `handoff.md` into the Phase 3d prompt; scopes Phase 1 research to the phase's `areas[]`/`goal`; writes to `phase-<N>-<slug>/` with phase-tagged metadata; flips roadmap status to `planned` only after `story-merge` succeeds. No roadmap.json → completely unaffected. Full rules: [pipeline-phases.md](references/pipeline-phases.md#rolling-wave-phase-selection).
