...
| `issues` | number[] | No | — | Forge-confirmed issue numbers this plan closes, as positive integers in ascending order. Written by `/aimi:plan`'s Issue Reference Confirmation Gate from candidates it resolved through `forge-issue-view` and the user then confirmed; read by `/aimi:open-pr` to render a `Closes` line per entry. Omitted entirely when nothing was confirmed — including every agent-mode run, which never confirms any — never `[]` and never `null`. |
