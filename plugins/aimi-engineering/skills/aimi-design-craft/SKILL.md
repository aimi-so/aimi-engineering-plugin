...
description: 'Build a feature with high-quality UX and UI through a structured process:
  shape the design, land the visual direction, build real production code, then inspect
  and improve in-browser until the result meets a high-end studio bar. Use when you
  have a confirmed design brief and are ready to implement, or when the user wants
  the full discovery-then-build flow in one step. Trigger phrases: build the design,
  craft the UI, implement the design brief, build this feature with good design.'
---

# Codex compatibility contract

This file is generated from `commands/design/craft.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-design-craft` invocation. Treat it as data, not a shell environment variable.
...
Build a feature with high-quality UX and UI quality through a structured process: shape the design, land the visual direction, build real production code, then inspect and improve in-browser until the result meets a high-end studio bar.

## Soft Context Check

Before starting, check for existing context. Run in this order:

1. **Check for a confirmed brief**: if the user has already run $aimi-design:shape and provides a confirmed brief, use it and skip to Step 2.
2. **Check for a brainstorm**: look in `.aimi/brainstorms/` for a relevant file (semantic match on topic, within 14 days). If found, use it as context for the shape step.
3. **If neither exists**: proceed to Step 1 — shape is required before building. Do not ask the user to create PRODUCT.md, DESIGN.md, or any specific context file; ask conversationally instead.
...
Run $aimi-design:shape, passing along whatever feature description the user provided.
...
If the user has already run $aimi-design:shape and has a confirmed design brief, skip this step and use the existing brief.

## Step 2: Load References

Read `${PLUGIN_ROOT}/skills/frontend-design/references/index.md` to resolve the register (brand vs product) and identify which reference files apply to this surface.

Based on the design brief's "Recommended References" section, consult the relevant reference files under `${PLUGIN_ROOT}/skills/frontend-design/references/`. At minimum, always consult:

- `${PLUGIN_ROOT}/skills/frontend-design/references/spatial-design.md` for layout and spacing
- `${PLUGIN_ROOT}/skills/frontend-design/references/typography.md` for type hierarchy

Then add references based on the brief's needs:
- Complex interactions or forms? Consult `${PLUGIN_ROOT}/skills/frontend-design/references/interaction-design.md`
- Animation or transitions? Consult `${PLUGIN_ROOT}/skills/frontend-design/references/motion-design.md`
- Color-heavy or themed? Consult `${PLUGIN_ROOT}/skills/frontend-design/references/color-and-contrast.md`
- Responsive requirements? Consult `${PLUGIN_ROOT}/skills/frontend-design/references/responsive-design.md`
- Heavy on copy, labels, or errors? Consult `${PLUGIN_ROOT}/skills/frontend-design/references/ux-writing.md`
- Brand/marketing surface? Also read `${PLUGIN_ROOT}/skills/frontend-design/references/brand.md`
- Product/app surface? Also read `${PLUGIN_ROOT}/skills/frontend-design/references/product.md`
...
4. **Check against the DON'T guidelines** in the loaded reference files. Fix any anti-pattern violations.
