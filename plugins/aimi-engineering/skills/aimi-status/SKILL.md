...
This file is generated from `commands/status.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-status` invocation. Treat it as data, not a shell environment variable.
...
- **`$PHASE_DIR_NAME` matches `^phase-[0-9]`** → **roadmap layout**. Derive the feature slug and the tasks-root directory (two and three levels above the tasks file, respectively):
  ...
  - If `AIMI_REQUEST` contains `--phase <id>` (a numeric phase id, e.g. `2` or `2.1`), skip Step 1/Step 2 and go to **Phase Detail View (`--phase <id>`)** below.
...
Rendered instead of Step 1/Step 2/Roadmap Summary whenever Step 0.5 detects a roadmap layout and `AIMI_REQUEST` contains `--phase <id>`.
