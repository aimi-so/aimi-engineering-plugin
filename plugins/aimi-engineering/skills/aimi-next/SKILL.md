...
This file is generated from `commands/next.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-next` invocation. Treat it as data, not a shell environment variable.
...
Execute the next pending story using a Codex-subagent agent.
...
**Parse `--container`/`--inline` Override.** Reached only when `PHASE_ID` is empty. Scan `AIMI_REQUEST` for an explicit `--container` or `--inline` token (mirrors execute.md's own Parse --container/--inline Override, and the `--base` extraction style just below):
...
2. **Parse `--base` Argument.** Scan `AIMI_REQUEST` for an explicit `--base <branch>` token (mirrors the `--phase <N>` extraction style used by `$aimi-execute` and the `--branch <name>` extraction style used by `$aimi-open-pr`):
...
5. **Create or reuse the container**, with CWD set to `CONTAINER_ROOT` so `worktree-manager.sh`'s own `git rev-parse --show-toplevel` resolves against the right repo. This is execute.md's **Create or Reuse a Container** with `EXEC_ROOT="$CONTAINER_ROOT"`, `EXEC_BRANCH="$BRANCH_NAME"`, `CONTAINER_BASE` as computed above — see that section for the full reuse/idempotency behavior. `$aimi-next` captures the exit code inline instead of letting the Bash tool call surface it, since its own STOP messaging (below) never uses `request_user_input`:
...
If `CREATE_EXIT` is non-zero, `$CREATE_OUTPUT` (just printed above) already names the occupying worktree — STOP here without any further checkout-conflict detection or remediation, and without `request_user_input`.
