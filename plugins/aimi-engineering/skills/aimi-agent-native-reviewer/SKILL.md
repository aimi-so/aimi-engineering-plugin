---
name: aimi-agent-native-reviewer
description: Reviews code to ensure agent-native parity — any action a user can take,
  an agent can also take. Use after adding UI features, agent tools, or system prompts.
---

# Codex compatibility contract

This file is generated from `agents/review/aimi-agent-native-reviewer.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-agent-native-reviewer` invocation. Treat it as data, not a shell environment variable.
