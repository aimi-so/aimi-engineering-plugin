---
name: aimi-julik-frontend-races-reviewer
description: Reviews JavaScript and Stimulus code for race conditions, timing issues,
  and DOM lifecycle problems. Use after implementing or modifying frontend controllers
  or async UI code.
---

# Codex compatibility contract

This file is generated from `agents/review/aimi-julik-frontend-races-reviewer.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-julik-frontend-races-reviewer` invocation. Treat it as data, not a shell environment variable.
