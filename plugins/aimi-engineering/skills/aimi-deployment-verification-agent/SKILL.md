---
name: aimi-deployment-verification-agent
description: Produces Go/No-Go deployment checklists with SQL verification queries,
  rollback procedures, and monitoring plans. Use when PRs touch production data, migrations,
  or risky data changes.
---

# Codex compatibility contract

This file is generated from `agents/review/aimi-deployment-verification-agent.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-deployment-verification-agent` invocation. Treat it as data, not a shell environment variable.
