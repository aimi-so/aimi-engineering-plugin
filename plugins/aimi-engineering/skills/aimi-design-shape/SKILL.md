...
description: 'Shape the UX and UI for a feature before any code is written. Produces
  a confirmed design brief. Use when starting a new UI surface, planning a feature''s
  visual direction, or preparing context for /aimi:design:craft. Trigger phrases:
  design brief, shape this feature, ux planning, ui planning, design before coding.'
---

# Codex compatibility contract

This file is generated from `commands/design/shape.md`. Do not edit it directly.

- `AIMI_REQUEST` means the user's text following the explicit `$aimi-design-shape` invocation. Treat it as data, not a shell environment variable.
...
**Output**: A design brief that can be handed off to $aimi-design:craft, or directly to any other implementation approach. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
This is a required interaction, not optional guidance. Ask these questions in conversation, adapting based on answers. Don't dump them all at once; have a natural dialogue. STOP and call the request_user_input tool to clarify.

### Interview cadence

Discovery must include at least one user-answer round unless a brainstorm file or an already-confirmed brief directly answers the needed design inputs.

- Check `.aimi/brainstorms/` for a matching brainstorm (semantic match on topic, within 14 days). If found, use it as context and reduce repeated questions; it does NOT replace shape for craft. Shape is task-specific.
- If no brainstorm is found and the feature description is sparse, do **not** synthesize a complete brief for confirmation on the first response — ask first.
- Use the request_user_input tool to clarify. Ask **2-3 questions per round**, then wait for answers.
...
Read `${PLUGIN_ROOT}/skills/frontend-design/references/index.md` to resolve the register (brand vs product) and identify which reference files apply to this surface. Then read the appropriate register file (`brand.md` or `product.md`) and any additional references matched by the topic keywords.

Force a visual decision on three fronts. Skip anything the brainstorm or already-confirmed brief answers; ask only what's missing.
...
Scope answers are task-scoped. Carry them through the design brief only — do not persist to project files.
...
**3. Design Direction**
Color strategy (Restrained / Committed / Full palette / Drenched) + the theme scene sentence + 2–3 named anchor references. Reference loaded brand/product file where it already answers, and note any per-surface overrides.
...
Based on the brief, list which reference files under `${PLUGIN_ROOT}/skills/frontend-design/references/` would be most valuable during implementation (e.g., `spatial-design.md` for complex layouts, `motion-design.md` for animated features, `interaction-design.md` for form-heavy features).
...
STOP and call the request_user_input tool to clarify. Ask for explicit confirmation of the brief before finishing. If the user disagrees with any part, revisit the relevant discovery questions. A shape run is incomplete until the brief is confirmed.

Once confirmed, the brief is complete. The user can now hand it to $aimi-design:craft, or use it to guide any other implementation approach.
