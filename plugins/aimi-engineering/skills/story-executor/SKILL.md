...
<result_json>
{
  ...
  "verify": {"exit": <int>, "preExit": <int>, "segments": <int>, "discriminating": <int>},  // optional, omit the WHOLE object when implementation.verify is absent or empty
...
- `verify` carries the evidence steps 1.5 and 4 ALREADY produce and today throw away: `exit` is step 4's real verify exit, `preExit` is step 1.5's pre-run exit, `segments` is how many entries `verify-probe` returned, and `discriminating` is how many of those had `discriminates` true. **`discriminating` counts `discriminates: true` and NOTHING ELSE.** An entry whose `discriminates` is `null` — the probe could not tell, because it declined to run a segment reading shell state the prelude never carried — counts toward `segments` and never toward `discriminating`, so a withheld verdict can never inflate the number of checks claimed to discriminate. There is deliberately no third count: `segments - discriminating` is `false` plus `null` together and was never a null tally, so do not read it as one. Omit the WHOLE object when `story.implementation.verify` is absent or empty — the missing key is the point, not a formatting detail: it is the signal, readable downstream, that the story ran with no verify at all.
- Inside the object, a number that does not exist is OMITTED and never invented. `segments` and `discriminating` leave together when the `verify-probe` verb was unavailable (an older CLI, or `python3` missing — the case step 1.5 already reports in one line and never treats as a failure); `exit` leaves when step 4 never ran at all, because the story failed before reaching it. **Zero is NEVER used to say "not measured"** — zero is a measured, successful exit, and using it as filler would let an empty record pass for a clean one, which is the exact defect this field exists to close.
- Prose OUTSIDE the block is for human debugging only. Keep it short; the orchestrator does not need it.
- Examples — success:
  ```
  <result_json>
  {"status":"ok","commit":"a6cd4678","tests":{"passed":7,"failed":0,"total":7},"typecheck":"ok","verify":{"exit":0,"preExit":1,"segments":12,"discriminating":9}}
  </result_json>
  ```
  Failure:
  ```
...
1.5. **Pre-run `story.implementation.verify` once, before implementing anything, and record the result.** Skip this step entirely when `implementation.verify` is absent or empty — it does not run and warns nothing, the same treatment step 4 gives the absent case (`implementation{}` is optional in schema v3.3). When present, run it exactly as written, from the working directory established in step 0c, with `TASKS_FILE_PATH` exported into its environment — a `verify` that needs its own tasks file (e.g. to read `metadata.baseRef`) can then resolve it directly rather than falling back to the shared `current-tasks` pointer a sibling split orchestrator's `init-session` may have since overwritten. If this pre-run exits ZERO — the verify already passes before any work exists — emit a warning, carried in the completion report, that the check does not discriminate the before-state from the after-state: a verify that passes both before and after proves nothing about what was done. This warning NEVER blocks, in any mode. Name its legitimate cause rather than accusing the verify of being broken: a story re-executed over partial work already done can genuinely pass this early run with nothing wrong — a warning that accuses wrongly gets ignored the third time it fires. Running the verify twice per story is a deliberate trade-off, accepted on purpose even though it doubles the run time of a story whose verify is a long suite: a verify that always passes reports success, and discovering that later costs more than the second run.
    ...
    **Choose the CLI that will do the probing before probing with it, and announce which one you chose.** Set `PROBE_TREE_CLI="$PWD/plugins/aimi-engineering/scripts/aimi-cli.sh"` — `$PWD` is the working directory step 0c established, and the path has to be absolute because `aimi-cli.sh` invoked by a relative path resolves its sibling Python modules against the directory it lands in after `find_aimi_root`, and can load another checkout's. When `[ -x "$PROBE_TREE_CLI" ]` — `-x`, never `-f` or `-e`, the same test `commands/execute.md`'s Creates Verification applies to its own container CLI, because a file that is present but not executable probes nothing — set `PROBE_CLI="$PROBE_TREE_CLI"` and print `probe-cli: $STORY_ID: probing with the aimi-cli.sh of the tree under test ($PROBE_TREE_CLI)` on stderr. Otherwise set `PROBE_CLI="$AIMI_CLI"` and print `probe-cli: $STORY_ID: no executable aimi-cli.sh in the tree under test ($PROBE_TREE_CLI) -- probing with the resolved CLI ($AIMI_CLI) instead`. **BOTH branches print** — a deliberate divergence from that precedent, which prints only on the fallback: there the silent branch is the rare one, here the tree under test is the COMMON case, so a silent preferred branch would hide precisely what this exists to show. On a feature branch the installed CLI is OLDER than the branch's own, and an old parser accepts an unknown flag in silence — so with no instrument named, two contradictory findings about the same flag are indistinguishable from each other. The prefix is `probe-cli: ` and not `verify-probe: ` because the verb itself writes `verify-probe: wrote <path>` to this same stream and you have to read that path back out of it; two producers under one prefix is confusion bought for nothing. **The preference is per-call and is NEVER persisted:** write it into no cli-path cache and teach no resolution layer about it. `write_global_cli_cache` and `_dev_dir_path` each refuse a path under `.worktrees/` on purpose, after a symlink bug that exited 127 — this stderr line is the substitute for persisting, not a complement to it.

    In the same step, run `"$PROBE_CLI" verify-probe $STORY_ID --tasks-file $TASKS_FILE_PATH` from that same working directory and list, in the completion report, every returned entry whose `discriminates` is `false` — under the sentence that these assertions already pass before the work. The pre-run above is at the granularity of the whole SCRIPT; this is at the granularity of the ASSERTION, and the two answer different questions. Under the `set -e` most verifies open with, the script stops at its first failing assertion and every assertion after it never runs at all — so a verify that exits non-zero here can still be carrying assertions that would have passed before a line was written, and the script-level pre-run says nothing about them. This listing is a WARNING and NEVER a block, exactly like the pre-run: a non-discriminating assertion is not automatically wrong. Some are guardrails by design — an assertion that a prior story's behaviour has not regressed is *supposed* to pass before and after, and it belongs in the report as one. What the list is for is the other kind, the assertion that asserts nothing about this story, which no one can see while it hides behind the first clause that fails. If the verb is unavailable (an older CLI, or `python3` missing), say so in one line and carry on — its absence never fails a story. **The verb writes this call's JSON array itself and names the file it wrote on one line of stderr (`verify-probe: wrote <path>`) — read that path and carry it; do not compose one.** The name is keyed on the tasks file's own stem, which is what keeps one plan's `US-002` off the previous plan's, and the file lands beside the tasks file rather than in any directory created here — both rules live in `tasks.py` now, where a suite reaches them. It is the PRE-change verdict step 4 hands back to this same verb once the real verify has run post-change. A write the verb could not make is one stderr line and nothing else: it never fails the probe, and stdout still carries the array.

    **A THIRD listing, under its own sentence and never merged into the one above: every returned entry whose `discriminates` is `null`.** A `null` is the probe saying it COULD NOT TELL. The segment reads shell state the probe's carried prelude never reproduced — the prelude carries plain assignments and `cd`s and nothing else, so a name set by an `eval`, a subshell or a function call is not there — and rather than measure it in a world that never existed, the probe DID NOT RUN IT AT ALL: its `exit` is `null` too. Its `unresolvedState` array names the variables that were missing, and the report must name them, not merely say a verdict was withheld — which state was absent is what points at the line that sets it, where "no verdict" points at nothing. A `null` entry may instead carry `unterminated` rather than `unresolvedState` — a quote, backtick, paren, brace or compound state left open at end-of-text, meaning the verify would not have run AT ALL as written; say so instead of hunting for an `unresolvedState` that is not there, and read `segments: 1, discriminating: 0` as "nothing was measured", never as a per-assertion count. Keeping this list apart from the `discriminates: false` one is the entire reason the third answer exists: a withheld verdict listed among the assertions that already pass reads as dead weight, and "already passes before the work" is the sentence that tells a reader to STOP LOOKING at the check it names. Two things a `null` does NOT mean, and the report must not imply either — a segment whose reads the prelude DID cover keeps its ordinary `true`/`false` verdict, unaffected by a `null` neighbour; and only SHELL state is lost, never disk, because each segment the probe runs is a real subprocess in the same working directory, so a fixture an earlier segment wrote to a FILE is still there for the ones after it. Warning only, exactly like the two lists it sits beside: a `null` entry never fails a story and never blocks a wave.

    **When probing would cost a full suite run, `--skip-matching <regex>` is the way out — and it is a trade, not a free win.** A segment the regex matches is not run, and it STAYS in the report carrying `discriminates: null` and `skipped: true`, so a skip can never be read as a segment the verify does not contain. Reach for it when the verify ends in a suite whose cost you already know and whose pass/fail this step adds nothing to — `python3 -m pytest`, `test-aimi-cli.sh` — because probing one of those costs its whole run time on top of the two runs steps 1.5 and 4 already pay. Do NOT reach for it just to make a slow probe finish sooner: every skipped segment is one this step can no longer warn you about, which is the opposite of why you ran it. It AVOIDS the cost rather than removing it. The same `null` now covers a segment that outlives the probe's per-segment ceiling: a run that was cut off measured nothing, so it is reported unknown — with `timedOut: true` — rather than as the verdict it used to be given, and like every other `null` it counts toward `segments` and never toward `discriminating`.

    **If step 4's verify later FAILS**, re-run `"$PROBE_CLI" verify-probe $STORY_ID --tasks-file $TASKS_FILE_PATH --previous-file <the path step 1.5 reported>` from the same working directory (that second run reports a path of its own, ending in `-post.json`, so it never overwrites the artifact it just read), and list, alongside the `discriminates:false` entries above rather than in a step of their own, every returned entry whose `unsatisfiable` is `true`. `discriminates` computed from one run cannot tell "the work is not done yet" from "this assertion can never pass" — both are non-zero — and the case this closes cost three fifteen-minute suite re-runs against a check that could never pass, because nothing told the two apart. A segment non-zero in BOTH runs is named possibly unsatisfiable: check the harness, not the code, rather than reported as ordinary unfinished work. This comparison is a WARNING like the rest of this step and NEVER overrides step 4's own FAIL verdict; if the verb or the saved file is unavailable, say so in one line and report the plain failure as before.

    **A story's own `implementation.verify` must NEVER name `verify-probe`.** This step RUNS that verify's segments, so a verify that calls the verb back re-enters the probe, and the CLI refuses rather than answering — the failure reported then says nothing about the story's own code. The refusal message is the one place that states the problem in full, and it names the two ways out: probe from a verify that does not itself call the verb (a sibling story's, or a throwaway fixture's), or call `probe_verify()` in `tasks.py` directly, which is not guarded. The mutual case is the same loop — story A's verify probing B while B's probes A, two ids and neither equal to the other — so the refusal catches it too.
...
7. **Emit the `<result_json>` block as the LAST element of your final message** (see "Result Contract" section above). The orchestrator parses this block as source of truth — prose outside the block is debugging only and is NOT consumed by the leader. **Populate `"verify"` from the numbers steps 1.5 and 4 have already produced**: step 4's exit becomes `exit`, step 1.5's pre-run exit becomes `preExit`, and the `verify-probe` array from step 1.5 — at the path that step reported — gives `segments` (its length) and `discriminating` (how many of its entries had `discriminates` true). Nothing new is executed here — this adds no third run of the verify, it stops discarding what was already measured. Omit the whole object when `implementation.verify` was absent or empty, and omit any single number that was never measured rather than filling it with zero (the Result Contract's Rules say which is which).
...
<execution_flow>
Bootstrap: re-read `$AIMI_CLI` from cache (per-call re-read one-liner — see `<task_pointer>` above), run `$AIMI_CLI get-story-context $STORY_ID --tasks-file $TASKS_FILE_PATH`, parse `{story, metadata, skills, designContext, skillsDropped}`. For each entry in `skills[]`, read its `.content` verbatim as additional project conventions (in addition to CLAUDE.md); a non-empty `skillsDropped[]` names a declared skill the 100KB cap excluded, and belongs in the completion report. Read `designContext.decisions` as design intent for UI-touching work; if `designContext.bundleGuidance` cites spec file paths (DesignSpec / BusinessSpec), use the Read tool to load those files before authoring implementation code. Read prototype files from `metadata.prototypePaths[]` and `story.implementation.prototypeAnchor` via Read tool (log missing, skip). **cd to the working directory `<worktree_context>` establishes; when WORKTREE_PATH is set and that cd fails, STOP and report `"status":"failed"` with the unenterable path in `failureCause` — never fall through to the directory you are already in, which belongs to the orchestrator or to a sibling story and would take this story's commit silently.** Then follow standard execution flow: read criteria → **pre-run `story.implementation.verify` once, before implementing anything, with `TASKS_FILE_PATH` exported into its environment** (skipped entirely when absent or empty; a ZERO exit here means the check does not discriminate before from after — warn in the completion report, NEVER block, and see step 1.5 above for the legitimate cause it must name, why the second run is worth its cost, and why the export matters — a `verify` may need to resolve its own tasks file rather than the shared `current-tasks` pointer) → **choose the probing CLI before probing with it: `PROBE_TREE_CLI="$PWD/plugins/aimi-engineering/scripts/aimi-cli.sh"` (absolute, so `aimi-cli.sh` loads its sibling Python modules from THIS checkout), and when `[ -x "$PROBE_TREE_CLI" ]` — `-x`, never `-f` or `-e` — set `PROBE_CLI="$PROBE_TREE_CLI"` and print `probe-cli: $STORY_ID: probing with the aimi-cli.sh of the tree under test ($PROBE_TREE_CLI)` on stderr, else set `PROBE_CLI="$AIMI_CLI"` and print `probe-cli: $STORY_ID: no executable aimi-cli.sh in the tree under test ($PROBE_TREE_CLI) -- probing with the resolved CLI ($AIMI_CLI) instead`. BOTH branches print — unlike the `commands/execute.md` precedent, which prints only its fallback — because here the tree under test is the COMMON case and a silent preferred branch hides the very thing this shows: a feature branch's CLI is newer than the installed one, and an old parser accepts an unknown flag in silence. The prefix is `probe-cli:` so it never collides with the verb's own `verify-probe: wrote` on the same stream, and the choice is per-call and NEVER persisted — `write_global_cli_cache` and `_dev_dir_path` refuse a `.worktrees/` path on purpose, after an exit-127 symlink bug** → **in the same step run `"$PROBE_CLI" verify-probe $STORY_ID --tasks-file $TASKS_FILE_PATH`, which writes its JSON array itself and names the file on one stderr line (`verify-probe: wrote <path>`) — carry that path, never compose one** (the name is keyed on the tasks file's stem so one plan's story id cannot land on another's, and the file goes beside the tasks file rather than in any directory created here; a failed write is one stderr line and never a failure) from the same directory (**a story's own verify must NEVER name this verb: probing RUNS its segments, so a verify that calls it back re-enters the probe and the CLI refuses instead of answering — probe from a sibling story's verify or a throwaway fixture, or call `probe_verify()` in `tasks.py`, which is not guarded**) and list every entry whose `discriminates` is `false` as assertions that already pass before the work (the pre-run above only sees the whole script; under `set -e` the script stops at its first failure and hides every assertion after it, so a verify that FAILS here can still be full of ones that prove nothing — warning only, never a block, and a guardrail assertion legitimately shows up in this list; an unavailable verb is one reported line, never a failure; **list every entry whose `discriminates` is `null` under a SEPARATE sentence of its own — the probe could not tell: the segment reads shell state the carried prelude never reproduced, so it was not run at all and its `exit` is `null` too. Name its `unresolvedState` contents so the report says WHICH state was missing, and never merge these into the already-passes list, where a withheld verdict would read as dead weight and stop a reader looking. A segment whose reads the prelude did cover keeps its ordinary verdict, and only shell state is lost — disk writes survive between segments. A `null` may instead carry `unterminated` — a quote or bracket left open at the end of the verify, meaning it would not have run at all — read `segments: 1, discriminating: 0` as nothing measured, not as an assertion count, rather than hunting for an absent `unresolvedState`. A `null` never fails a story, never blocks a wave, and never counts toward `discriminating`**; **`--skip-matching <regex>` is the way out when probing would cost a full suite run: a matching segment is not run, stays in the report as `discriminates: null` with `skipped: true`, and a segment outliving the per-segment ceiling answers the same null with `timedOut: true`. It AVOIDS the cost rather than removing it, and every skipped segment is one this step can no longer warn about — so reach for it for a verify ending in a suite whose cost you already know, never merely to make a slow probe finish sooner**; **if the real verify FAILS later, re-run this same `"$PROBE_CLI" verify-probe` call with `--previous-file <the path reported above>`, and list every `unsatisfiable:true` entry here too — that run reports a `-post.json` path of its own and leaves the artifact it read alone — a segment non-zero in both runs is possibly unsatisfiable: check the harness, not the code, rather than unfinished work, and this never overrides the FAIL verdict below**) → implement (follow `story.tasks[]` as the ordered recipe when present; treat `"Wire <X> into <Y>"` entries as mandatory cross-story integration steps; AC remains the completion gate; **follow `story.implementation.approach` as the strategy the planner chose — optional in schema v3.3, so absent or empty means invent none, and a divergence from a present one goes in the report's `deviations[]` with its reason, never unsaid**) → run `story.implementation.verify` exactly as written from the working directory established above, `TASKS_FILE_PATH` still exported (optional in schema v3.3 — absent/empty means do exactly what you already do and invent no command; a non-zero exit FAILS the story, report and do NOT commit — apply the verify-probe comparison above before writing that report) → **when WORKTREE_PATH is set, check `git branch --show-current` ENDS in this story's id (`-<STORY_ID>`) before staging anything; a mismatch is a failure, not a warning — do not stage, do not commit, and report the branch found beside the id expected** → commit. If `story.verification.strategy == "visual"` OR `metadata.prototypePaths` is non-empty or `story.implementation.prototypeAnchor` is set, run the Visual Source-of-Truth Protocol (V1/V2/V3) before writing code. Stage only story-related files (never `-A` or `.`). Commit format: `git commit -m "type(scope): Story title"`. Verify with `git log -1 --oneline`. On commit failure: report immediately, do not retry. Do NOT update tasks file — caller handles status. When a reference artifact is declared, run the Reference-Artifact Parity Pass before committing (see full named section above). **End your final message with the `<result_json>` block per the Result Contract section — the orchestrator parses ONLY that block; prose outside is debugging only.** Populate `"verify"` in it from numbers already in hand — the real verify's exit as `exit`, the pre-run's exit as `preExit`, and the saved `verify-probe` array's length and `discriminates:true` count as `segments` and `discriminating`; no extra run of anything. Omit the whole object when `implementation.verify` was absent or empty, and omit a number that was never measured rather than writing zero for it.
</execution_flow>
...
- [ ] **`verify` populated in that block** from the step 1.5 and step 4 numbers (whole object omitted when the story had no `verify`; an unmeasured number omitted, never zero-filled)
