...
- `verify` carries the evidence steps 1.5 and 4 ALREADY produce and today throw away: `exit` is step 4's real verify exit, `preExit` is step 1.5's pre-run exit, `segments` is how many entries `verify-probe` returned, and `discriminating` is how many of those had `discriminates` true. Omit the WHOLE object when `story.implementation.verify` is absent or empty — the missing key is the point, not a formatting detail: it is the signal, readable downstream, that the story ran with no verify at all.
