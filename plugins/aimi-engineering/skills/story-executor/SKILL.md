...
7. **Emit the `<result_json>` block as the LAST element of your final message** (see "Result Contract" section above). The orchestrator parses this block as source of truth — prose outside the block is debugging only and is NOT consumed by the leader. **Populate `"verify"` from the numbers steps 1.5 and 4 have already produced**: step 4's exit becomes `exit`, step 1.5's pre-run exit becomes `preExit`, and the `verify-probe` array step 1.5 saved to `$(dirname "$TASKS_FILE_PATH")/verify-probe-$STORY_ID.json` gives `segments` (its length) and `discriminating` (how many of its entries had `discriminates` true). Nothing new is executed here — this adds no third run of the verify, it stops discarding what was already measured. Omit the whole object when `implementation.verify` was absent or empty, and omit any single number that was never measured rather than filling it with zero (the Result Contract's Rules say which is which).
