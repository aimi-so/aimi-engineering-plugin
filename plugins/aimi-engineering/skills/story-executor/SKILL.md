...
<execution_flow>
Bootstrap: re-read `$AIMI_CLI` from cache (per-call re-read one-liner — see `<task_pointer>` above), run `$AIMI_CLI get-story-context $STORY_ID --tasks-file $TASKS_FILE_PATH`, parse `{story, metadata, skills, designContext, skillsDropped}`. For each entry in `skills[]`, read its `.content` verbatim as additional project conventions (in addition to CLAUDE.md); a non-empty `skillsDropped[]` names a declared skill the 100KB cap excluded, and belongs in the completion report. Read `designContext.decisions` as design intent for UI-touching work; if `designContext.bundleGuidance` cites spec file paths (DesignSpec / BusinessSpec), use the Read tool to load those files before authoring implementation code. Read prototype files from `metadata.prototypePaths[]` and `story.implementation.prototypeAnchor` via Read tool (log missing, skip). Then follow standard execution flow: read criteria → **pre-run `story.implementation.verify` once, before implementing anything, and record the result** (skip entirely when `implementation.verify` is absent or empty — same treatment the absent case gets below, since `implementation{}` is optional in schema v3.3; when present, run it exactly as written from the working directory established above; if this pre-run exits ZERO, emit a warning carried in the completion report that the check does not discriminate before from after — a verify passing both before and after proves nothing about what was done — but this warning NEVER blocks, in any mode, and must name the legitimate cause: a story re-executed over partial work already done can genuinely pass here with nothing wrong, since a warning that accuses wrongly gets ignored the third time it fires; this doubles the verify's run count for a story that has one, a deliberate trade-off since a verify that always passes reports success and discovering that later costs more than the second run) → implement (follow `story.tasks[]` as the ordered recipe when present; treat `"Wire <X> into <Y>"` entries as mandatory cross-story integration steps; AC remains the completion gate) → run `story.implementation.verify` exactly as written from the working directory established above (optional in schema v3.3 — absent/empty means do exactly what you already do and invent no command; a non-zero exit FAILS the story, report and do NOT commit) → commit. If `story.verification.strategy == "visual"` OR `metadata.prototypePaths` is non-empty or `story.implementation.prototypeAnchor` is set, run the Visual Source-of-Truth Protocol (V1/V2/V3) before writing code. Stage only story-related files (never `-A` or `.`). Commit format: `git commit -m "type(scope): Story title"`. Verify with `git log -1 --oneline`. On commit failure: report immediately, do not retry. Do NOT update tasks file — caller handles status. When a reference artifact is declared, run the Reference-Artifact Parity Pass before committing (see full named section above). **End your final message with the `<result_json>` block per the Result Contract section — the orchestrator parses ONLY that block; prose outside is debugging only.**
</execution_flow>
