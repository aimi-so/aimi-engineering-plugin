...
**Measured, with an as-of marker, and re-measure rather than quote.** As of `b2edf0c` on branch `ref/jq-nao-forge-para-python` (stories 01–09 landed), `scripts/aimi-cli.sh` carries **232 real `jq` invocations**: 237 matches of the bare word on non-comment lines, minus 5 that are not invocations at all — `check_jq`'s own `command -v jq` plus its two install-hint strings, and one prose mention inside `cmd_help`'s usage text. A count with no as-of marker beside it is how a stale figure outlives the tree it described, so if you need the number, take it again.
...
| 13 | **version/cache `jq -n`** — `cmd_prime_cache` (7), `cmd_check_version` (4), `cmd_cleanup_versions` (2) | All `jq -n`: a JSON printf with correct escaping, over values this script already holds. Rewriting them as `printf` would reproduce the unescaped hand-built JSON these verbs exist to have replaced. These four verbs are also the ones that LOCATE `aimi-cli.sh`, so a Python crossing here would make finding the CLI depend on having found it — see `cmd_prime_cache`'s header comment, which is the normative home of that decision. |
...
**The mechanical form of the principle is that only EIGHT of the 232 open a file at all**, and every one of them is named in the table above: `validate_story_exists`, the two `jq empty` probes, `_split_detect_describe`, `_roadmap_require`, `_roadmap_require_contracts`, `cmd_roadmap_write_handoff` and `cmd_version`. Everything else either constructs JSON from values the script already holds or reads back something the same function just produced. If you are weighing a new `jq` line, that is the question to ask of it: does a document arrive here that this script did not write in this call?
