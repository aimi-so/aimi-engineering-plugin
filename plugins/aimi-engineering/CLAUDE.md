...
**`python3` is a runtime requirement for these verbs, not only a test-time one.** Thirteen of the fourteen shell out to `scripts/roadmap.py`, which owns `roadmap.json`'s document logic; each calls `check_python3()` first and fails with an install hint when it is absent. `estimate-payload` is the one exception and stays pure Bash + jq. Outside this group only `story-merge` needs python3 (it shells out to `scripts/story_merge.py`); every other verb stays pure Bash + jq and still runs on a host without it. The repo-level statement of this requirement, including what it means for an OpenCode install, is in the top-level `CLAUDE.md` § Testing.
