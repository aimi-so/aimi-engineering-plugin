...
> Key fields: `schemaVersion` ("3.2"), `metadata{title,type,branchName,maxConcurrency,researchDepth}`, `userStories[]{id(US-NNN),title,description,acceptanceCriteria,status,dependsOn,project,wave,implementation{files,approach,verify},verification{strategy,status,url,expect},gate{type,status,prompt,options}}`
