...
- **`story-merge`** — Consolidates per-story JSON staging files written by Pass 2 sub-agents into a single validated `tasks.json`. Key behaviors:
  ...
  - **Flags**:
    ...
    - `--split legacy|full-stack` — `full-stack` emits two files (frontend + backend); `legacy` (default) emits one file. Partition rule: group stories by normalized `.project` (trim whitespace, strip one trailing slash, blank treated as absent), then decide each group's side by strict majority vote of its members' own file-pattern/title heuristic verdict (ties go to backend). When fewer than 2 distinct normalized `.project` values are present across the merge — including the monorepo case where no story has `.project` set at all — the majority vote is skipped and every story falls back to its own per-story heuristic verdict (monorepo guard).
