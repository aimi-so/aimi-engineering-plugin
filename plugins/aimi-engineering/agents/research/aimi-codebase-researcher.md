...
Every factual claim in the findings body (not the pointer-block return in step 5 above, which stays exactly 3 summary bullets + `sections`) resolves to one of exactly three forms — no bare assertions:
