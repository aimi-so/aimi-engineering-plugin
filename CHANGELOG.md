...
Every field named below belongs to a `forge-*` verb first shipped in 1.120.0. **No action is required of any caller.** These verbs are consumed only by this plugin's own command markdown (`open-pr.md`, `review.md`, `validate-bug.md`, `execute.md`) and the `resolve-pr-parallel` skill scripts, all of which are updated in this same release; they are not a slash-command syntax anyone types. Phase 1 is still an open pull request against `feat/forge-abstraction` and has never been on `main`, so no external consumer has had the opportunity to depend on 1.120.0's shapes. This subsection exists so that if anyone did, they can see exactly what moved.
