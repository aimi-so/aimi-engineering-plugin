...
- **`validate-stories` now refuses a multi-repo story whose `implementation.verify` does not begin with the exact literal `cd <project> && `.** A tasks.json that validated cleanly yesterday can fail today. `plan.md` and `agents/workflow/aimi-story-expander.md` already told an author to write that prefix (the documentation half, below); this closes the enforcement half in the same story loop that already refuses an absolute, traversing, metacharacter-bearing or invalid-charset `project` — a multi-repo story whose verify command never `cd`'d into its own project used to validate green and fail the moment `/aimi:execute` actually ran it. The match is pinned to the literal itself, not to "begins with `cd <project>`": a trailing slash, a subshell, a quoted path, and a leading env assignment are all readable commands and all refused the same as no prefix at all, because none of them is the string the docs tell an author to write. `project "."` and no `project` stay exempt, and an absent or empty `verify` is untouched — no rule governs that field's presence. Issue #105.
- **A phase whose stories are all completed-or-skipped now reads has-work `false` and is demoted in `roadmap-claim`'s auto ranking, the same way a fully-completed phase already was.** `ground_truth` had no branch for this shape and fell through to `in_progress` — a completed phase carrying one deliberately skipped story got un-completed on every reconcile, and a phase nobody had touched yet read as though execution was mid-flight on it. Two branches close it: every story completed-or-skipped now resolves to `completed`, every story still pending now resolves to `planned`. A bare `/aimi:execute` may claim a different phase than it would have before upgrading. Issue #102's own "not affected" analysis of `has_work_map` predates the skipped branch and does not cover this case. Issues #102, #112.
...
- **Version call — MINOR, not MAJOR, argued from both manifests read live and confirmed to agree at `1.120.0` before any edit.** New CLI behaviour ships that a caller could not invoke before — `detect-parent-branch`'s `ambiguous-decoration` answer, `roadmap.json`'s new `integrationBranch` field below, and `validate-stories`' new rejection — which excludes PATCH. This repository reserves MAJOR for the one-time, repo-wide `tasks.json` schema rewrite; `1.92.0` is the precedent for shipping a genuinely breaking change under MINOR with an explicit disclosure instead, which is the shape `validate-stories`' new rejection takes here: no command's syntax or output format moved, but an existing multi-repo `tasks.json` that passed yesterday can refuse today.
...
- **`implementation.verify`'s working directory is now stated once in `plan.md` and cited once in `agents/workflow/aimi-story-expander.md`, not answered five different ways.** Five parallel story-expander agents had answered the same unasked question — which directory does `verify` run from in a multi-repo story — five different ways, because the story-shape block's placeholder never said; two of five happened to emit a working `cd <project> &&` prefix and three did not. This is the documentation half of the rule the enforcement above now checks. Issue #105.
