...
- **`test_check_version`'s Test 2, `test_check_version_quiet`, and `test_check_version_backward_compat`'s Test 1 now build their own throwaway plugin cache instead of reading the developer's ambient one.** All three exercise `check-version`'s "missing" branch (`aimi-cli.sh:10199-10206` — no stored `cli-path`), but on a host with an empty `~/.claude/plugins/cache` (no `aimi-engineering` install for the glob to find), `cmd_check_version` fell through to its "unknown" branch (`:10185-10190`) instead, and the four assertions expecting `"status": "missing"` (plus the stderr warning) failed. Each of the three now builds a `mktemp -d` root, seeds it with `_make_cached_version` — the pattern `test_check_version`'s own Test 1 already used for the "current" case — and pins `CLAUDE_CONFIG_DIR`/`AIMI_CONFIG_DIR` at it for that one CLI invocation, so the glob always resolves and "missing" is reached deterministically on any host. No assertion's expected value changed and `EXPECTED_ASSERTIONS` is untouched: an already-registered assertion moving from failing to passing adds no new one. `test_check_version_backward_compat`'s Test 2/Test 3 (the ambient-cache-conditional current/stale sub-cases) are unchanged.
