...
**Why this is MINOR, not PATCH.** Eight forge verbs that previously answered `no adapter for forge "gitea" yet` now route to `tea`: `forge-auth-status`, `forge-pr-view`, `forge-issue-view`, `forge-pr-create`, `forge-pr-edit`, `forge-issue-create`, `forge-pr-review-threads` and `forge-resolve-review-thread`. SemVer makes added functionality MINOR however small the rest of the release is. Nothing is removed, renamed or invalidated: no command syntax and no output shape changed, every GitHub and GitLab path behaves as it did in 1.123.0, and every Gitea path replaces an error envelope rather than a working one.
...
- **1.123.0's `## [1.123.0] - 2026-08-05` heading had been silently destroyed by a merge, and is restored here.** `b96b4a2` wrote that heading; merge commit `708ab45` (PR #96 into `feat/forge-abstraction`) resolved the CHANGELOG conflict by keeping both sides' bodies and dropping the single `## [...]` line that existed only on the phase-3 side. No body text was lost — only the heading — so the branch spent phase 4 claiming `1.123.0` in `plugin.json` and `marketplace.json` while `CHANGELOG.md`'s top released heading read `## [1.122.0]` and the whole of 1.123.0's text sat orphaned under `## [Unreleased]`. **The exact failure the release story exists to prevent had therefore already happened once, silently.** The restored section was diffed against `git show b96b4a2:CHANGELOG.md` and is byte-identical to what 1.123.0 shipped.
  - **The dropped heading had also swallowed a phase-4 entry, which is the part a version-string check would never have caught.** With no `## [1.123.0]` line in the file, the last `### Fixed` under `## [Unreleased]` *was* 1.123.0's, so `6f9a259` — phase 4's fail-open-guard fix — appended its release note into the previous release's section. Restoring the heading is what made that visible; the entry now sits under 1.124.0 where it belongs. The duplicated `### Added`/`### Changed` pairs under `[Unreleased]` were the same seam and were deliberately **not** consolidated: merging them would have folded the GitLab adapter's notes into the Gitea adapter's and erased the boundary between two releases permanently.
...
**Why this is MINOR, not PATCH.** Phase 2 added three capabilities that did not previously exist — the per-repository account store, the `forge-account-select` verb, and the command-layer picker — and phase 3 adds an entire second forge adapter behind verbs that previously returned `no adapter for forge "gitlab" yet`. SemVer makes added functionality MINOR however small the rest of the release is. Nothing is removed, renamed or invalidated: every GitHub path behaves as it did in 1.122.0, and every GitLab path replaces an error envelope rather than a working one.
