...
>
> **Why PATCH.** The defect is the headline — both symptoms are the same disagreement seen from two angles, and closing it is a bug fix in the ordinary sense. The counterweight is stated rather than buried: `roadmap-init` genuinely accepts less than it did. An identity carrying `;`, `|`, `&` or a bare `$` used to be written silently and now aborts the command, which is a tightening, and by the letter of `plugins/aimi-engineering/CLAUDE.md` a tightening argues for a number a reader notices. It was still set at PATCH because every identity now refused was already unverifiable downstream: the contract reader refused that whole class, so writing one only deferred the identical rejection to the next `/aimi:plan`. The backtick case makes the point measurably — against the pre-fix CLI, `roadmap-init` fed `` creates: ["`cmd_foo` (a backticked name)"] `` *already* exited 1, with `is not a usable artifact identity: empty once the description is stripped`, because the sanitizer emptied the identity before the guard judged it. For that shape this release replaces an unreadable failure with a legible one rather than rejecting input that used to work. This plugin argues its release levels case by case instead of applying the MAJOR/MINOR/PATCH definitions mechanically; the two entries below did the same, in the other direction.

### Fixed

- **The writer and the reader now agree on what a legal artifact identity is (#99, #85).** The roadmap sanitizer removed `$(`, backticks, code fences and HTML tags but left `;`, `|`, `&` and a bare `$` untouched, while the contract reader refused that whole class — so `roadmap-init` wrote phases that `validate-contracts` then hard-failed, stopping `/aimi:plan`'s Pre-Expansion Contract Gate on a roadmap the CLI itself had produced. Both sides now read one shared table instead of two that overlapped by accident.
...
> Six phases of forge abstraction, consolidated into one release. The forge-specific knowledge that was scattered across commands now sits behind a normalized verb contract, and **three forges sit behind it**: GitHub, GitLab, Gitea/Forgejo.
>
> **Why one version and not six.** This work was developed on a feature branch that cut five intermediate release numbers — 1.120.0 through 1.124.0 — none of which was ever published. `main` went straight from 1.119.3 to this release, the repository carries no git tags, and no consumer ever saw an intermediate number. Six headings for work that ships in one merge is noise, so the per-phase narratives are kept as subsections below rather than as separate releases.
>
> **Why MINOR and not MAJOR.** The `### Breaking` subsection under phase 1.1 reshapes several `forge-*` verb envelopes. Every field it names belongs to a verb that did not exist in 1.119.3, the last published version — so there is no upgrade path on which those shapes ever existed, and the subsection is a disclosure for anyone who built against the branch, not a migration notice. Those verbs are consumed only by this plugin's own command markdown and skill scripts, all updated in this same release; they are not syntax anyone types.
>
> **Why MINOR and not PATCH.** Each phase added capability that did not previously exist: the forge abstraction and its verbs, per-repository account selection, and two further adapters. Nothing was removed or renamed relative to 1.119.3.
