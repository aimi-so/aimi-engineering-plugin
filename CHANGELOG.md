...
- **`story-merge --split full-stack` now picks its split axis from the plan's own layout instead of always splitting frontend/backend (resolves issue #72, `aimi-so/aimi-engineering-plugin`).** Before either writer runs, the merged story array is counted for distinct normalized `project` values (trim whitespace, strip one trailing slash, blank/absent treated as null). **Two or more distinct values → PROJECT axis:** one output file per repo (`<base>-<project-slug>-tasks.json`), ordered lexicographically by normalized project path, each carrying its own derived `branchName` — and no frontend/backend decision is made anywhere on that path. Project-less stories route to the `"."` root group; ids are reassigned `US-001..US-M` in contiguous per-group blocks so they stay unique across the whole N-file set; two project paths that slugify to the same basename hard-fail the merge before any file is written, as does a derived branch name failing `^[a-zA-Z0-9][a-zA-Z0-9/_-]*$`. **Fewer than two distinct values → SIDE axis:** the two-file `*-frontend-tasks.json` / `*-backend-tasks.json` writer, unchanged. Single-repo plans and monorepos — no story sets `project`, or every story shares one value — keep the previous behavior byte-identical. The axis exists because separate repositories are a layout fact, not a question the frontend/backend heuristic is capable of answering.
- **Every PROJECT-axis split file carries a self-describing `metadata.splitGroup` marker** = `{project, index, total, siblings[]}` — its own project routing key, its 1-based position, the total file count, and the paths of its N−1 siblings. This is what `/aimi:plan` Phase 4 and `/aimi:execute` Step 0.9 detect on, replacing filename-pattern guessing. Detection order is load-bearing: the marker pass runs first and wins, and the legacy suffix rule runs only when no candidate carries a marker — otherwise a repo whose path slugifies to the basename `frontend` or `backend` would be misread as the legacy pair. SIDE-axis and legacy files have no `splitGroup` key, and none is invented for them.
- **`story-merge`'s return contract gains a third shape, and `/aimi:plan` now consumes it instead of re-deriving filenames.** On the PROJECT axis story-merge prints an N-element array of `{path, project, branchName, storyCount}`; the SIDE-axis `{frontend, backend, frontend_stories, backend_stories}` object and the legacy `{merged, stories}` object are unchanged. Phase 3e captures stdout as `MERGE_RETURN`, and Phase 4 / Phase 4.5 read their file list from it rather than string-concatenating `$TASKS_PATH` with two hard-coded suffixes — the surviving projects, their slugs, and their count are computed inside story-merge and are unknowable to the caller any other way. Phase 4 additionally resolves each PROJECT-axis branch's base branch per repo via `detect-default-branch --project`, and must merge its patch **into** the existing `metadata` object so `splitGroup` survives verbatim.
...
- **`/aimi:execute` runs N per-repo split files, in both the flat and the phase-mode flow.** Step 0.9's paired-split path is generalized from two named Frontend/Backend slots into a loop over the active file set — two for a legacy pair, N for an N-repo project split — still spawning every member in a single tool-call turn. Each split file's worktree and container are rooted at that file's own repo (`metadata.splitGroup.project` resolved against `AIMI_ROOT`), never at `AIMI_ROOT` itself, and the completion report's totals are summed with `git -C` per repo against that repo's own default branch. Step 1.7's Phase-Mode Paired Split takes the same generalization and reuses the identical two-pass detection order, scoped to the phase's own directory.
- **TC18, TC20, TC21, and TC22 in `test-aimi-cli.sh` were rewritten on purpose — read the four flipped assertions as expected, not as a regression.** All four pinned the majority-vote behavior removed below, so all four went red the moment that vote was deleted: superseded, not broken. TC18 now asserts that a two-distinct-project fixture yields one file per project and no side files at all; TC20 generalizes the two-file bipartition invariant to an N-way one, reading the file set off the writer's own return value; TC21 asserts `.project` survives verbatim on every story of every per-project file and that no internal bookkeeping leaks into the output; TC22 keeps its trailing-slash fixture but swaps the tie-goes-to-backend assertion for a normalization-boundary one (both spellings of the same path land in one group). Each rewritten case carries a leading `SUPERSEDED, NOT BROKEN` comment naming exactly what it used to assert and pointing here.
- **Version call — MINOR, not MAJOR, recorded so it is auditable rather than assumed.** This release does change `story-merge --split full-stack`'s output format for multi-repo plans: a different number of files, different filenames, and a changed return contract. Under this repo's own "MAJOR: breaking changes to command syntax or output format" rule that could be read as MAJOR. MINOR is chosen on the identical precedent set by PR #68 / 1.113.0 — which also changed split output for existing multi-repo plans (partition-by-`project` plus a new `smellWarnings` entry type) and shipped as MINOR — and because `story-merge` is an internal CLI verb invoked by `/aimi:plan`, not a user-facing command surface, while the single-repo/monorepo path here is byte-identical to 1.115.0.
...
- **Issue #73 reframed and addressed: multi-repo + container + split was a diagnosability defect, not a corruption one (`aimi-so/aimi-engineering-plugin`).** Verified empirically before any fix was written — the combination corrupts nothing. `cd "$AIMI_ROOT"` followed by a worktree create under a non-git parent folder exits **128** with a bare `fatal: not a git repository` and creates nothing at all; the failure is total and immediate, and what it fails to do is name the phase, the layout, or the fix. Two changes address that. First, each split file now owns exactly one repo, so its container is created under that repo rather than under the non-git root — which removes the failure from the flat multi-repo split path entirely. Second, for what remains, `/aimi:execute` refuses clearly instead of letting git produce the opaque message: a guard placed before the first `cd` and before any `$WORKTREE_MGR create`, covering phase mode under a non-git root and the case where a multi-repo plan converges on a single surviving repo and therefore lands as a two-file frontend/backend split under that same non-repo root. The refusal names the detected layout, the exact failure that would otherwise follow, and what to do about it.
- **Stale prose describing the deleted majority vote as current behavior** in `skills/task-planner/SKILL.md`, its `references/pipeline-phases.md` copy, and `story-merge`'s own `--help` text now describes the axis choice instead. `plugins/aimi-engineering/CLAUDE.md`'s Tasks File Schema likewise documents both `cross-file-dep-dropped` shapes and their mutual exclusivity. Documentation only, no behavior change. No README.md edit accompanies this release — no command, skill, or agent was added or removed, so the component-count tables are unaffected.
