...
### Added
- `roadmap-set-status --status cancelled`: a new terminal phase status alongside `completed`, for a phase abandoned rather than finished. `pending`/`planned` → `cancelled` needs no `--force`; `in_progress`/`verification_failed` → `cancelled` do, the same as any other out-of-order transition. `completed` → `cancelled` is refused even with `--force` — a completed phase's `creates` were already delivered, and this transition has no override. `cancelled` → `pending` (reopening) is refused without `--force` and succeeds with it; `cancelled` → `cancelled` is an idempotent success; every other departure from `cancelled` is refused even with `--force`. Reaching `cancelled` clears the phase's claim in the same locked write, the same as reaching `completed`. A cancelled dependency satisfies a downstream phase's `dependsOn` the same way a completed one does, and a cancelled phase is never claimable, by auto-selection or by an explicit `--phase` override.
- `roadmap-amend-phase --name <text> --slug <slug>`: renames a phase's name and slug before it runs. Refused — by name, before anything on disk changes — unless the phase's own `status` is `pending` or `planned`, its `claim` is `null`, and every story in its own phase tasks file is still `pending`. When the amended `slug` changes the computed `dir` (`phase-<id>[-<slug>]`), the same locked call validates the new `dir` against a collision, moves `<old_dir>` to `<new_dir>` on disk, rewrites that phase's own tasks file's `metadata.phase.dir`, and rewrites `metadata.branchName`'s slug segment when present — and rolls all three back to their exact pre-amend state if the `roadmap.json` write itself then fails. Result JSON's `move` key is `{from, to}` when a directory moved, `null` otherwise.
- `remove-story <id> --reason <text> [--tasks-file <path>]`: withdraws a pending, dependency-free story from a plan and records why, so a scope cut is a single auditable command instead of a hand-edited `tasks.json`. Refuses, inside one locked crossing and in this order: the id not found; the story not `pending` (naming its actual status); the story having dependents (naming every one, not just the first); the story being the only entry left in `userStories` (pointing at `roadmap-set-status --status cancelled` for a phase-scoped file, `archive-task` for a flat one); or `--reason` missing/empty. On success the story is removed and one entry is appended to `metadata.decisions` (`anchor: "removeStory:<id>"`, `source: "removeStory"`), with both the story's title and the reason sanitized through `sanitize.py`'s `rm_sanitize` at a 2000-character cap.
- `sanitize.py`'s `rm_sanitize_report(value, maxlen)` classifies what the prose sanitizer did to a value — `rewritten` and/or `truncated`, or nothing — by calling the existing `rm_sanitize` twice (unclamped, then at the cap) rather than duplicating its regexes. `roadmap-init` and `roadmap-amend-phase` now report this for every free-text field they sanitize: a touched field draws one stderr warning line naming the phase, field and change kind(s), and the result JSON's new `sanitized[]` array carries `{phase, field, index, changes[], before, after}` per touched field — character counts only, never the field's own value before or after.

### Changed
- `validate-contracts`, `roadmap-sweep`, `roadmap-reconcile` and `list-archivable-phases` (the roadmap read behind `list-archivable`) now treat `cancelled` as terminal alongside `completed`. `validate-contracts`' duplicate-creates scan and `roadmap-amend-phase`'s own collision check both exempt a cancelled phase's `creates`, so a live phase may reuse an identity a cancelled sibling abandoned; a cancelled phase's `creates` still report `not-delivered` on a scoped needs check, since the dependency edge is satisfied but the artifact was never delivered. `roadmap-sweep` drops a cancelled phase's own entries from `orphanCreates`/`deferredNeeds` but leaves every other phase's reference sets unfiltered. `roadmap-reconcile` now skips a `cancelled` phase entirely, on the status check alone before its tasks file is ever read, and never corrects it back toward whatever its stories' ground truth would read.
- `roadmap-amend-phase`'s amendable-field list grows from six fields (`goal`, `successCriteria`, `creates`, `needs`, `areas`, `branch`) to eight, adding `name` and `slug`; `dir` now refuses with its own message naming `slug` as the way to change it, instead of falling into the generic phase-identity refusal `id`/`dependsOn` still get.
- `/aimi:plan`, `/aimi:status` and `/aimi:execute` render `cancelled` phases correctly instead of treating them as still-open work. The single- and multiple-roadmap pickers count and display cancelled phases alongside completed ones (the cancelled count omitted when zero), the single-roadmap auto-adopt predicate and the zero-eligible phase report both recognize `cancelled` as finished, `roadmap-claim`'s auto mode treats a cancelled `dependsOn` phase as satisfied the same as a completed one, and `/aimi:status`'s blocker line no longer lists a cancelled dependency or prints a cancelled phase's own status as anything other than `cancelled`.
