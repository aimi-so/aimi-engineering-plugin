...
**No action is required of anyone upgrading, and the blast radius is zero.** Every field named below belongs to a `forge-*` verb that did not exist before 1.120.0 — and 1.120.0 has never been published. The last released version is **1.119.2**: that is what `main` carries, in both `marketplace.json` and `plugin.json`, and it is `main`'s topmost dated CHANGELOG heading. The 1.120.0 release commit is not an ancestor of `main`; it is reachable from exactly one feature branch and its `origin` mirror. `feat/forge-abstraction`, the base this work targets, is itself still at 1.119.2. So the shapes reshaped below were introduced on an unmerged branch, shipped to no one, and the upgrade path from the last published version is untouched — a consumer moving from 1.119.2 to 1.121.0 never saw 1.120.0's envelopes at all.
